...
import {
  ...
  filterSidebarGroupsByConsoleVisibility,
  getDefaultConsoleSidebarVisibility,
  normalizeConsoleSidebarVisibility,
  sidebarGroups,
  type ConsoleSidebarVisibility,
} from './sidebarConfig.js';
export { sidebarGroups } from './sidebarConfig.js';
...
function AppShell() {
  ...
  const [consoleSidebarVisibility, setConsoleSidebarVisibility] = useState<ConsoleSidebarVisibility>(() => getDefaultConsoleSidebarVisibility());
  const [consoleSidebarVisibilityLoaded, setConsoleSidebarVisibilityLoaded] = useState(false);
  ...
  const visibleSidebarGroups = consoleSidebarVisibilityLoaded
    ? filterSidebarGroupsByConsoleVisibility(sidebarGroups, consoleSidebarVisibility)
    : sidebarGroups;
  ...
  useEffect(() => {
    ...
    const loadConsoleSidebarVisibility = async () => {
      try {
        const runtimeSettings = await api.getRuntimeSettings();
        if (cancelled) return;
        setConsoleSidebarVisibility(normalizeConsoleSidebarVisibility(runtimeSettings?.consoleSidebarVisibility));
        setConsoleSidebarVisibilityLoaded(true);
      } catch {
        if (cancelled) return;
        setConsoleSidebarVisibility(getDefaultConsoleSidebarVisibility());
        setConsoleSidebarVisibilityLoaded(true);
      }
    };

    void loadConsoleSidebarVisibility();

    const handleVisibilityChanged = () => {
      void loadConsoleSidebarVisibility();
    };

    window.addEventListener(CONSOLE_SIDEBAR_VISIBILITY_CHANGED_EVENT, handleVisibilityChanged as EventListener);
    return () => {
      cancelled = true;
      window.removeEventListener(CONSOLE_SIDEBAR_VISIBILITY_CHANGED_EVENT, handleVisibilityChanged as EventListener);
    };
  }, [authed]);
