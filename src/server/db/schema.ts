...
export const downstreamSites = sqliteTable('downstream_sites', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  name: text('name').notNull(),
  hostSiteId: integer('host_site_id').notNull().references(() => sites.id, { onDelete: 'cascade' }),
  baseUrlOverride: text('base_url_override'),
  authMode: text('auth_mode').notNull().default('session-admin'),
  adminCredentialCipher: text('admin_credential_cipher').notNull(),
  ...
  hostSiteIdx: index('downstream_sites_host_site_id_idx').on(table.hostSiteId),
  enabledIdx: index('downstream_sites_enabled_idx').on(table.enabled),
  hostSiteNameUnique: uniqueIndex('downstream_sites_host_site_name_unique').on(table.hostSiteId, table.name),
}));

export const downstreamSiteChannels = sqliteTable('downstream_site_channels', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  downstreamSiteId: integer('downstream_site_id').notNull().references(() => downstreamSites.id, { onDelete: 'cascade' }),
  remoteChannelId: text('remote_channel_id').notNull(),
  remoteName: text('remote_name').notNull(),
  remoteType: integer('remote_type'),
  remoteGroup: text('remote_group'),
  balance: real('balance'),
  rawConsumedQuota: real('raw_consumed_quota'),
  derivedConsumedUsd: real('derived_consumed_usd'),
  ...
  siteChannelUnique: uniqueIndex('downstream_site_channels_site_channel_unique').on(table.downstreamSiteId, table.remoteChannelId),
  siteIdx: index('downstream_site_channels_site_id_idx').on(table.downstreamSiteId),
  syncedAtIdx: index('downstream_site_channels_synced_at_idx').on(table.syncedAt),
}));

export const reconciliationRuns = sqliteTable('reconciliation_runs', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  sourceType: text('source_type').notNull().default('global'),
  sourceId: integer('source_id'),
  scopeType: text('scope_type').notNull(),
  windowStart: text('window_start').notNull(),
  windowEnd: text('window_end').notNull(),
  modelScope: text('model_scope').notNull().default('family'),
  ...
  sourceScopeIdx: index('reconciliation_runs_source_scope_idx').on(table.sourceType, table.sourceId, table.scopeType),
}));

export const reconciliationFacts = sqliteTable('reconciliation_facts', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  runId: integer('run_id').notNull().references(() => reconciliationRuns.id, { onDelete: 'cascade' }),
  factType: text('fact_type').notNull(),
  sourceType: text('source_type').notNull().default('global'),
  sourceId: integer('source_id'),
  hostSiteId: integer('host_site_id').references(() => sites.id, { onDelete: 'set null' }),
  downstreamSiteId: integer('downstream_site_id').references(() => downstreamSites.id, { onDelete: 'set null' }),
  supplierSiteId: integer('supplier_site_id').references(() => sites.id, { onDelete: 'set null' }),
  timeBucketType: text('time_bucket_type').notNull(),
  timeBucketStart: text('time_bucket_start').notNull(),
  timeBucketEnd: text('time_bucket_end').notNull(),
  modelRequestedRaw: text('model_requested_raw'),
  modelActualRaw: text('model_actual_raw'),
  modelCanonical: text('model_canonical'),
  modelFamily: text('model_family').notNull().default('other'),
  supplierKey: text('supplier_key'),
  supplierConfidence: real('supplier_confidence'),
  requestCount: integer('request_count'),
  tokenCount: integer('token_count'),
  billedQuota: real('billed_quota'),
  costUsd: real('cost_usd'),
  usageConfidence: real('usage_confidence'),
  priceConfidence: real('price_confidence'),
  rawPayload: text('raw_payload'),
  createdAt: text('created_at').default(sql`(datetime('now'))`),
}, (table) => ({
  runIdx: index('reconciliation_facts_run_id_idx').on(table.runId),
  runTypeIdx: index('reconciliation_facts_run_type_idx').on(table.runId, table.factType),
  runWindowFamilyIdx: index('reconciliation_facts_run_window_family_idx').on(table.runId, table.timeBucketStart, table.modelFamily),
}));
...
export const reconciliationResults = sqliteTable('reconciliation_results', {
  ...
  modelFamily: text('model_family').notNull().default('other'),
  modelCanonical: text('model_canonical'),
  downstreamBilledTokens: integer('downstream_billed_tokens').notNull().default(0),
  downstreamBilledCostUsd: real('downstream_billed_cost_usd').notNull().default(0),
  metapiObservedTokens: integer('metapi_observed_tokens').notNull().default(0),
  metapiObservedCostUsd: real('metapi_observed_cost_usd').notNull().default(0),
  upstreamConsumedQuota: real('upstream_consumed_quota').notNull().default(0),
  upstreamConsumedCostUsdDerived: real('upstream_consumed_cost_usd_derived').notNull().default(0),
  deltaTokens: integer('delta_tokens').notNull().default(0),
  deltaCostUsd: real('delta_cost_usd').notNull().default(0),
  deltaRate: real('delta_rate'),
  status: text('status').notNull().default('warning'),
  confidenceScore: real('confidence_score').notNull().default(0),
  explanationCodes: text('explanation_codes'),
  explanationText: text('explanation_text'),
  createdAt: text('created_at').default(sql`(datetime('now'))`),
  updatedAt: text('updated_at').default(sql`(datetime('now'))`),
}, (table) => ({
  runIdx: index('reconciliation_results_run_id_idx').on(table.runId),
  runWindowFamilyIdx: index('reconciliation_results_run_window_family_idx').on(table.runId, table.timeBucketStart, table.modelFamily),
}));

export const reconciliationModelMappings = sqliteTable('reconciliation_model_mappings', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  pattern: text('pattern').notNull(),
  modelCanonical: text('model_canonical').notNull(),
  ...
  patternUnique: uniqueIndex('reconciliation_model_mappings_pattern_unique').on(table.pattern),
  enabledPriorityIdx: index('reconciliation_model_mappings_enabled_priority_idx').on(table.enabled, table.priority),
  familyIdx: index('reconciliation_model_mappings_family_idx').on(table.modelFamily),
}));
